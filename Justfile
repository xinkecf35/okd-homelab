...
apply-day-1-okd: _tf_init_global_iac
    ...
    ansible-playbook "{{ global_ansible_dir }}/day-1-bootstrap-playbook.yaml" --inventory "{{ global_ansible_dir }}/inventory/hosts" --vault-id "{{ ansible_vault_id }}@{{ ansible_vault_password_helper }}" --extra-vars="@${OKD_GLOBAL_IAC_TF_OUTPUT_JSON_FILE}" --extra-vars=@{{ global_ansible_secrets_vars_file }} --ask-become
